...
### 1.5 公告與狀態端點（2026-08-27 實地驗證）

全部是 `POST https://www.tpex.org.tw/www/zh-tw/bond/{code}`，body 只需 `response=json`。

| code | 內容 | 性質 | 欄位 |
|---|---|---|---|
| `convDelist` | 最近下櫃 | **滾動快照**，47 筆／約 3 個月 | 代碼、簡稱、下櫃日期 |
| `convSearch` | 最近上櫃 | **滾動快照**，387 筆／約 3 年 | 發行機構代碼、名稱、債券名稱、掛牌日期、**MOPS 條款連結** |
| `cbMode` | 變更／分盤／停止交易 | **滾動快照** | 證券代號、名稱、變更交易、分盤交易、撮合循環、停止交易 |
| `putProvision` | 賣回權 | **滾動快照**，41 筆 | 代號、名稱、賣回日期、賣回金額、賣回收益率 |
| `cbSuspend` | 停止轉換 | 檔案索引（逐日） | 資料日期 + `/storage/.../RSdrs002.{YYYYMMDD}-C.csv` |
| `cbCoupon` | 債息 | 檔案索引（逐年，21 個年度） | 資料年度 + `/storage/.../rsta0236.{年}-C.csv` |

**前四個是滾動視窗，不每天存就永久消失**，由 `scripts/snapshot.py` 每日抓取並累積成 `data/snapshots/{名稱}_history.csv`（含首見／末見日期）。後兩個跟日行情一樣可回補歷史。

### 1.5b 其他 CB 頁面（端點已定位，參數待抓）
...
### 2.4 t120sg01 —— 已找出缺的參數

先前回「年月參數錯誤」，缺的是 **`monyr_reg`（申報年月，格式 `YYYYMM`）**。

答案來自櫃買的「最近上櫃」端點（`convSearch`）—— 它每一筆都附上完整的 MOPS 連結：

```
https://mopsov.twse.com.tw/mops/web/t120sg01
  ?issuer_stock_code=3088      發行人股票代號（CB 代號前 4 碼）
  &bond_id=30882               CB 代號
  &bond_yrn=2                  期別（CB 代號第 5 碼起）
  &bond_subn=$M00000001        固定值
  &bond_kind=5                 5 = 轉換公司債
  &monyr_reg=202607            ← 先前缺的就是這個
  &come=2&step=0&firstin=ture&encodeURIComponent=1&TYPEK=&pg=&tg=
```

注意 `firstin=ture` 是櫃買原本的拼字（不是 `true`），照抄即可。

這同時證實了前一份交接文件對 `bond_yrn` / `bond_subn` 的推導規則是對的，只是漏記了 `monyr_reg`。

**尚未做**：用這組參數實際抓取並解析條款內容。`convSearch` 提供了 387 檔的現成連結，可直接當作起點。
