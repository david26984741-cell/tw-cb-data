...
以下全部來自前一份交接文件，**尚未在本機驗證**，跑 `scripts/probe.py` 前不要當成事實。

- `t120sg01` 缺 `bond_yrn` 或 `bond_subn` 會回「債券期參數錯誤」。推導規則：`issuer_stock_code` = CB 代號前 4 碼、`bond_yrn` = 第 5 碼起、`bond_subn` = `$M00000001`
- 包裹下載關鍵是 `step=12`（查詢是 11），且至少要給一個查詢條件，否則回「除預設項目外，請至少輸入一項查詢項目」。用 `issuer_stock_code_1=1000&issuer_stock_code_2=9999` 等於全撈
- 包裹下載**無法指定月份**，只能做每月增量
- **欄位名不一致**：包裹下載叫「認股價格」（`發行時認股價格`／`最新認股價格`／`最新認股價格生效日期`），`t120sg01` 叫「轉(交)換價格」
- 包裹下載回 Big5 HTML 表格、日期為**西元**；`t120sg01` 為**民國**
- MOPS 附件 PDF 會 404，發行辦法要用就當下鏡像存檔
- `mopsov.twse.com.tw` 的 robots.txt disallow `/mops/` —— 所以 MOPS 一律在本機跑，不放 GitHub Actions
